...
- 用户负责需求范围、设计方向、完整设计和发布四类不可自动代替的决定。

外部 Skill 只作为受控依赖使用。跨客户端编排、PRD、UX 架构、设计方向比较、设计交付和阶段门禁由 AgentFlow 自己的 Skill 实现。

### 1.1 实现快照

截至 2026-07-15，M0 状态内核、M1 Codex Host 契约和 M2 产品到设计控制面已经落地：RunState 持久化 Task、Worker、原生任务 ID、能力快照、prompt 哈希、结构化结果和独占资源；AgentFlow MCP 提供 Worker 生命周期、Artifact 合约校验、Figma 单 Writer lease、逐调用 operation mutex，以及可恢复的 live capability preflight。测试覆盖两个并行实现 Worker、Supervisor 重启恢复、依赖 Review Worker，以及 Product Brief→PRD→UX→A/B/C→设计方向 Gate 的状态迁移。测试中的 Figma 返回值仅是 fixture，不是线上 Figma 执行证据。

Codex 已实跑一次 `prepare→native spawn→bind→collect` Worker 闭环。Codex、Cursor、VS Code 三份项目级 MCP 配置均已生成并通过静态 `doctor`，但 Cursor 和 VS Code 的原生 Worker Adapter 尚未实跑。当前会话没有暴露 Figma MCP 工具、resource/template 或 `figma-use` Skill，S04 live preflight 因此保持 blocked；项目没有声称创建任何 Figma 文件、node、截图或画布 Artifact，也不通过 GUI 自动化或伪造 OAuth 绕过阻断。

M3 已进入“契约与执行控制面”阶段：获批计划可原子物化为带 wave 屏障的 S11 Task DAG；计划固定 Git branch/base revision，`worker_dispatch_prepare` 从持久化 Task 确定性生成含可解析 Artifact locator 的 prompt，并在一个 revision 中保存 claim、经核验的 workspace 和 prepared Worker。当前 Run 已真实完成一次 Codex `prepare→native spawn→bind→collect`。此外，自动化 E2E 已在临时 Git 仓库中用两个真实 worktree 和 commit 跑通 plan materialize、双 Task prepare/bind/collect、按计划 cherry-pick 和 S12 Integration Report，并验证伪造 revision/path、dirty worktree 和错误验证命令会 fail closed；该 E2E 的 Worker 执行仍是测试驱动，不冒充双 native Codex 实跑。S05-S08、完整 S09-S15 真实项目执行闭环以及 Cursor/VS Code 原生 Adapter 仍在后续范围内。
...
表中 Skill 是 Pipeline `0.4.0` 的目标声明，不代表所有外部能力都已在线。当前已实现 S09、S10、S12、S13、S14、S15 的 AgentFlow Wrapper 与相应合约；S11 使用已实现的 worktree Wrapper。但尚未跑通一次真实的 S09-S15 全链路，S05-S08 的三个自建生产设计 Wrapper 仍是计划项。`release-plan` 只描述可审核的发布和回滚步骤，只有真实执行及证据齐备后，`agentflow-completion-verifier` 才能生成 `final-manifest`。
...
`runtime.db` 由中央 MCP Server 独占写入并加入 `.gitignore`。`state.json`、事件摘要、审批和业务产物可提交到 Git，用于恢复和审计。Worker 不直接写数据库。
...
CLI 和 MCP 使用同一个 Core，不实现两套状态逻辑。
...
上面的 `--agent '*'` 只用于一次性的审计 staging，方便观察 CLI 会为哪些客户端生成目录。正式项目不能保留同名 Skill 的 `.cursor/skills`、`.codex/skills` 和 `.agents/skills` 多份副本，否则兼容目录扫描可能产生重复 Skill。正式安装由 `agentflow skills sync` 完成：

1. 下载到 `.agentflow/vendor/skills/<source>/<commit>/`。
2. 运行静态扫描、许可证检查和人工 review。
3. 只向公共 `.agents/skills/` 物化一份批准版本。
4. 只有目标客户端无法读取公共目录时，Adapter 才创建客户端专属映射。
5. 检测到 Codex 或 Cursor 已安装官方 Figma Plugin 时，复用插件中的官方 Skill 和 MCP，不再复制第二份；VS Code 使用公共 Skill 加 workspace MCP 配置。
...
三端项目级 MCP 配置生成与静态 `doctor` 已通过；Codex 原生 Worker 路径已实跑，Cursor 与 VS Code 原生 Worker Adapter 仍未实现或实跑。
...
`skills-lock.json`、Release Gate Skill 和 Release Plan 合约已经存在；自动化依赖升级、真实发布权限交接、部署/回滚执行与运行手册仍待完成。

## 20. MVP 验收场景

给出需求：“做一个面向小团队的项目管理 Web 应用”。系统必须：

1. Supervisor 使用 brainstorming 逐步澄清，不写代码。
2. 生成 PRD，用户批准后才进入 UX。
3. UX Worker 输出角色、旅程、页面清单和异常状态。
4. 三个 concept Worker 输出 A/B/C brief，但只有一个 Figma Writer 写画布。
5. 用户选择 B 后，系统只为 B 建立完整设计系统和页面。
6. 设计冻结后才生成工程计划。
7. 前端、后端和测试 Worker 在独立 worktree 并行工作。
8. Integration Worker 按依赖合并并运行全量验证。
9. QA Worker 对照 Figma 做桌面与移动视觉检查及无障碍检查。
10. Security Worker 完成发布前审查。
11. 未获得用户发布批准时，系统不能调用生产部署工具。
12. 任意时刻关闭主对话并重新打开，`agentflow status` 能恢复当前 Stage、Worker 和 Gate。
...
6. 何时为当前仓库建立首次 baseline commit，以启用真实双 Worker worktree 前向测试。
