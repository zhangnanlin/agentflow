...
> 状态：v0.4.0 已发布；v0.5.0 自适应流程与原生协同版本已完成源码集成，package、tag 与部署仍受发布门禁约束
> 调研日期：2026-07-15
...
v0.3.0 已经实现一次用户级 GitHub `npx` 安装、独立 CLI/MCP ESM bundle、三端用户配置安全合并、全局自动路由 Skill、逐调用项目根解析，以及带锁、journal 和请求幂等记录的懒初始化，并已发布到 GitHub `main` 与不可变 `v0.3.0` tag。普通项目变更先调用 `run_start_or_resume`，只在目标项目创建轻量 `.agentflow` 状态；纯问答、代码解释、只读检查、状态查询、简单非写命令，以及只推送已有提交或标签的安全 Git 同步不会初始化项目。force push、ref 删除、历史重写、package 发布、release 创建、deployment、migration 与包含文件修改的混合请求仍必须进入 AgentFlow。
...
| S14 | Release | `agentflow-release-gate`、CI/CD、finishing branch | `release-plan`；源码 ref 可即时校验，package/production 使用独立发布 Worker | 用户批准发布或先前明确的一次性源码推送授权 |
...
表中 Skill 是 Pipeline `0.5.0` 的目标声明，不代表所有外部能力都已在线。当前已实现 S09、S10、S12、S13、S14、S15 的 AgentFlow Wrapper 与相应合约；S11 使用已实现的 worktree Wrapper。一个内部非 UI Run 已完成真实 S09-S15 和本地发布证据闭环，S05-S08 的三个自建生产设计 Wrapper、live Figma 以及外部生产发布仍未验证。`release-plan` 只描述可审核的发布和回滚步骤，只有真实执行及证据齐备后，`agentflow-completion-verifier` 才能生成 `final-manifest`。
...
| `agentflow-release-gate` | 已实现 | qa report、immutable revision、release kind | `release-plan` | 安全源码推送可确定性执行；package/production 仍需独立 Worker 和明确 Gate |
...
`structured_choice_request` 是只读的通用选择入口：只接受 1 至 3 个互相独立的非敏感单选问题，每题 2 至 5 个选项；推荐项只展示，不生成默认答案。宿主支持 MCP form elicitation 时显示可点击选项，已经暴露的原生结构化控件也可作为等价入口；不支持时只回退一次简洁文本，并且不会重复已经接受的回答。

`gate_decision_request` 不接受调用方自定义问题、选项或决议。它从持久化 Run 读取当前待处理人工 Gate，在不持有 Run 锁的情况下等待一次明确交互，再用原 revision、用户 actor、幂等输入哈希和当前 Artifact 哈希完成现有 Gate 事务。拒绝、取消、超时、断连、无效响应、过期 revision、Artifact 变化和并发冲突都不修改 Gate。

Pipeline `0.5.0` 当前注册 10 个强类型 Artifact kind：
...
已有全局安装升级到 0.5.0 时只需重新执行同一个主入口并按需重启宿主。各项目不重新安装，不新增 MCP server 条目，也不新增 OAuth 流程；Figma OAuth 仍只在声明了对应能力的 UI Stage 中按需发生。
...
“看起来不错”“继续看看”等模糊表达不能视为批准。批准必须记录对象哈希和明确决议。支持结构化输入时，每个 Gate 通过一次明确交互完成；推荐、沉默、取消、超时或其他 Artifact 的批准都不能授权当前 Gate。
...
| 宿主不支持结构化输入 | 只显示一次简洁文本回退；接受回答后不再重复询问 |
| Gate 询问被取消或发生并发冲突 | 保持 Gate、revision 和 Artifact 绑定不变，重新读取持久化状态后再决定是否询问 |
...
### M5：依赖治理与发布（v0.5.0 源码候选已集成，外部发布未授权）

- Skill lock、更新 PR 和审计报告。
- OAuth、tool allowlist 和日志脱敏。
- Release Gate、回滚和运行手册。

`skills-lock.json`、固定 commit staging、独立 GitHub `npx` 发行包、Host Setup/回滚手册、Release Gate Skill 和 Release Plan 合约已经存在。v0.4.0 已完成结构化选择、Artifact 绑定 Gate、集成、QA、GitHub `main` 推送与不可变 tag 发布。v0.5.0 候选加入自适应流程、原生 Worker 协同、共享限流与确定性清理；它在完成系统 QA 和新的 Release Gate 之前不得创建 tag、发布 package 或部署。后续安全 Git 同步可在严格 preflight/postflight 后直接执行；自动化依赖升级、package 发布、真实发布权限交接以及外部生产部署/回滚仍必须经过明确授权与证据核验。
