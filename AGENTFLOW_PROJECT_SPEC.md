...
> 状态：v0.3.0 已发布；v0.4.0 结构化输入候选开发中，尚未授权发布
> 调研日期：2026-07-15
...
已有全局安装升级到 0.4.0 时只需重新执行同一个主入口并按需重启宿主。各项目不重新安装，不新增 MCP server 条目，也不新增 OAuth 流程；Figma OAuth 仍只在声明了对应能力的 UI Stage 中按需发生。
...
### M5：依赖治理与发布（v0.4.0 候选开发中，生产发布未授权）

- Skill lock、更新 PR 和审计报告。
- OAuth、tool allowlist 和日志脱敏。
- Release Gate、回滚和运行手册。

`skills-lock.json`、固定 commit staging、独立 GitHub `npx` 发行包、Host Setup/回滚手册、Release Gate Skill 和 Release Plan 合约已经存在。v0.3.0 已完成集成、QA、GitHub `main` 推送与不可变 tag 发布。v0.4.0 候选加入结构化选择与一次交互的 Artifact 绑定 Gate；它在完成集成、系统 QA 和新的 Release Gate 之前不得推送、打 tag、发布或部署。后续安全 Git 同步可在严格 preflight/postflight 后直接执行；自动化依赖升级、package 发布、真实发布权限交接以及外部生产部署/回滚仍必须经过明确授权与证据核验。
